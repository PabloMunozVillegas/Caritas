...
import { getSucursal, getEmpresaId } from '../api/apiGet';
    ...
    useEffect(() => {
        const fetchSucursalAndEmpresa = async () => {
            try {
                ...
                const sucursalResponses = await getSucursal(user.sucursal, token);
                setSucursalDetails(sucursalResponses);

                const empresaResponse = await getEmpresaId(sucursalResponses.empresa, token);
